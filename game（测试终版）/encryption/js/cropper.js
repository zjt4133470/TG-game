...
(function (factory) {
    ...
    Cropper.prototype = {
        constructor: Cropper, init: function () {
            ...
            if (this.isRotated) {
                this.isRotated = false;
                rotated = getRotatedSizes({width: image.width, height: image.height, degree: rotate});
                ...
                if (aspectRatio !== canvas.aspectRatio) {
                    ...
                    if (rotate % 180) {
                        rotated = getRotatedSizes({width: naturalWidth, height: naturalHeight, degree: rotate});
            ...
            this.$canvas.css({width: canvas.width, height: canvas.height, left: canvas.left, top: canvas.top});
            this.renderImage();
            ...
            this.$cropBox.css({width: cropBox.width, height: cropBox.height, left: cropBox.left, top: cropBox.top});
            if (this.isCropped && this.isLimited) {
                this.limitCanvas(true, true);
            }
            if (!this.isDisabled) {
            ...
            this.$preview.each(function () {
                ...
                $this.css({width: newWidth, height: newHeight}).find('img').css({
                    width: width * ratio,
                    height: height * ratio,
            ...
            if (action) {
                ...
                this.action = '';
                this.trigger(EVENT_CROP_END, {originalEvent: originalEvent, action: action});
            }
            ...
            var data;
            if (this.isBuilt && this.isCropped) {
                data = {left: cropBox.left, top: cropBox.top, width: cropBox.width, height: cropBox.height};
            }
            return data || {};
